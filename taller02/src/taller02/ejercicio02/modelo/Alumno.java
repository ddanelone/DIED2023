...
import taller02.ejercicio02.excepciones.NoPUedeAsignarDocenteException;
...
public class Alumno {
	 ...
	 * CURSO y ALUMNO.
	 */
	public void inscribir(Inscripcion i) throws Exception {
		long contador = inscripciones.stream()
		        .filter(unaInscripcion -> unaInscripcion.estado()==Estado.REGULAR || unaInscripcion.estado() ==Estado.LIBRE)
		        .count();
		    
		    if (contador < 3) {
		        inscripciones.add(i);
		    } else {
		        throw new NoPuedeInscribirAlumnoException("El aluno no puede ser inscripto al Curso.");
		    }
	}
	}
